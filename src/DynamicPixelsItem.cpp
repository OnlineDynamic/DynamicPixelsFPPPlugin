...
DynamicPixelsItem::DynamicPixelsItem(std::string const& ip, unsigned int startChannel) :
    m_ipAddress(ip),
    m_port(9999),
...
std::string DynamicPixelsItem::getInfo() {
    const std::string cmd = "{\"system\":{\"get_sysinfo\":{}}}";
    return sendCmd(cmd);
}

void DynamicPixelsItem::serializeUint32(char (&buf)[4], uint32_t val) {
    buf[0] = (val >> 24) & 0xff;
    buf[1] = (val >> 16) & 0xff;
    buf[2] = (val >> 8) & 0xff;
    buf[3] = val & 0xff;
...
void DynamicPixelsItem::decrypt(char *input, uint16_t length) {
    uint8_t key = 171;
    uint8_t next_key;
...
void DynamicPixelsItem::encrypt(char *data, uint16_t length) {
    uint8_t key = 171;
...
void DynamicPixelsItem::encryptWithHeader(char *out, char *data, uint16_t length) {
    char serialized[4];
...
std::string DynamicPixelsItem::sendCmd(std::string const& cmd) {
    try {
...
uint16_t DynamicPixelsItem::sockConnect(char *out, const char *ip_add, int port, const char *cmd, uint16_t length) {
    if(m_issending)
    {
        return 0;
    }
