...
DynamicPixelsPSUSwitch::DynamicPixelsPSUSwitch(std::string const& ip, unsigned int startChannel, int plug_num) :
    DynamicPixelsItem(ip,startChannel),
    m_w(0),
    m_plug_num(plug_num)
{
...
std::string DynamicPixelsPSUSwitch::GetConfigString() const
{
...
bool DynamicPixelsPSUSwitch::SendData( unsigned char *data) {
    try
    {
        if(m_unreachable){
            return false;
        ...
        std::thread t(&DynamicPixelsPSUSwitch::outputData, this, w );
        t.detach();
        //outputData(w );
        return true;
    }
...
void DynamicPixelsPSUSwitch::outputData( uint8_t w ) {
    if(w >= 127){
        setRelayOn();
    } else {
        setRelayOff();
...
std::string DynamicPixelsPSUSwitch::getDeviceId(int plug_num) {
...
std::string DynamicPixelsPSUSwitch::appendPlugData(std::string cmd) {
